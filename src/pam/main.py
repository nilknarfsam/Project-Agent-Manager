...
from pam.project_bootstrap import (
    ProjectBootstrap,
...
def cmd_onboard(args: argparse.Namespace) -> int:
    ...
    print(f"[onboard] Caminho: {path}")
    if args.force:
        print("[onboard] Modo --force: arquivos existentes podem ser sobrescritos.")

    try:
        slug = args.project_name
        result = bootstrap.onboard(
            path,
            project_slug=slug,
            force=args.force,
        )
    except ProjectBootstrapError as exc:
        print(f"[onboard] Erro: {exc}", file=sys.stderr)
        return 1

    print("[onboard] Estrutura OS4AI aplicada com sucesso.")
    print(format_bootstrap_report(result))
    print(
        "Próximo passo:\n"
        f"  python -m pam.main plan {result.project_slug}"
    )
    return 0


def cmd_create_project(args: argparse.Namespace) -> int:
    """Cria novo projeto PAM-native com estrutura OS4AI."""
    bootstrap = ProjectBootstrap()

    print(f"[create-project] Stack: {args.stack} | Nome: {args.name}")
    if args.path:
        print(f"[create-project] Diretório pai: {args.path}")
    if args.force:
        print("[create-project] Modo --force ativo.")
    ...
    try:
        ...
        print(f"[create-project] Erro: {exc}", file=sys.stderr)
        return 1

    print("[create-project] Projeto criado com sucesso.")
    print(format_bootstrap_report(result))
    print(
        "Próximo passo:\n"
        f"  cd {result.repo_path}\n"
        f"  python -m pam.main plan {result.project_slug}"
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    onboard_parser = subparsers.add_parser(
        "onboard",
        help="Aplica estrutura OS4AI em repositório existente",
    )
    onboard_parser.add_argument(
        "path",
        help="Caminho do repositório (ex.: C:\\src\\projects\\auratime)",
    )
    onboard_parser.add_argument(
        "--project-name",
        default=None,
        help="Slug do projeto no PAM (padrão: nome da pasta normalizado)",
    )
    onboard_parser.add_argument(
        "--force",
        action="store_true",
        help="Sobrescrever arquivos gerados por template se já existirem",
    )
    onboard_parser.set_defaults(func=cmd_onboard)

    create_parser = subparsers.add_parser(
        "create-project",
        help="Cria novo projeto PAM-native (flutter, python, electron)",
    )
    create_parser.add_argument(
        "stack",
        choices=["flutter", "python", "electron"],
        help="Stack inicial do projeto",
    )
    create_parser.add_argument(
        "name",
        help="Nome do projeto (ex.: auratime, log-analyzer)",
    )
    create_parser.add_argument(
        "--path",
        default=None,
        help="Diretório pai onde criar o projeto (padrão: inferido dos YAMLs)",
    )
    create_parser.add_argument(
        "--force",
        action="store_true",
